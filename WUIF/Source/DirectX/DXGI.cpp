...
DXGIResources::DXGIResources(Window &win) :
    ...
    window(win)
{
    ...
    *SampleDesc.Count - number of multisamples per pixel - flip effects can't use multi-sampling
     so set to 1
    ...
    *Format = structure that describes the display forma - DXGI_FORMAT_B8G8R8A8_UNORM is the most
    ...
    *SwapEffect = value that describes the presentation model - DXGI_SWAP_EFFECT_FLIP_SEQUENTIAL
     ...
     (cannot be used with multisampling)
       NB: MS documenation states that DXGI_SWAP_EFFECT_FLIP_SEQUENTIAL is not available on Win 7
...
void DXGIResources::GetDXGIAdapterandFactory()
{
    ...
    if (!dxgiFactory)
    {
        ...
        if (App::winversion > OSVersion::WIN8)
        {
            if (App::GFXflags & FLAGS::D3D12) //already tested for Win10 in WUIF.cpp/InitResources
            {
                ...
                if (pfnd3d12getdebuginterface)
                {
                    ...
                    AssertIfFailed(pfnd3d12getdebuginterface(IID_PPV_ARGS(debugController.ReleaseAndGetAddressOf())));
            ...
            CREATEDXGIFACTORY2 createfactory2 = reinterpret_cast<CREATEDXGIFACTORY2>(
                GetProcAddress(GetModuleHandle(_T("dxgi.dll")), "CreateDXGIFactory2"));
            _ASSERTE(createfactory2);
            ThrowIfFailed(createfactory2(DXGI_CREATE_FACTORY_DEBUG,
        ...
        HINSTANCE lib = LoadLibrary(_T("dxgidebug.dll"));
        if (lib)
        {
            using PFN_DXGI_GET_DEBUG_INTERFACE = HRESULT(WINAPI *)(REFIID, void**);
            PFN_DXGI_GET_DEBUG_INTERFACE pfndxgigetdebuginterface =
                reinterpret_cast<PFN_DXGI_GET_DEBUG_INTERFACE>(GetProcAddress(
                    lib, "DXGIGetDebugInterface"));
    ...
    //factory exists - if we have an adapter release and reenumerate
    dxgiAdapter.Reset();
    ...
    if (App::GFXflags & FLAGS::D3D12)
    {
                ...
                ++adapterIndex)
        {
            ...
            PFN_D3D12_CREATE_DEVICE pfnd3d12createdevice =
                reinterpret_cast<PFN_D3D12_CREATE_DEVICE>(GetProcAddress(App::libD3D12,
                    "D3D12CreateDevice"));
            _ASSERTE(pfnd3d12createdevice); //D3D12CreateDevice should have loaded
        ...
        if (!(App::GFXflags & FLAGS::D3D11)) //if D3D12 only throw an error
        {
            throw WUIF_exception(_T("This application requires Direct3D 12 and there is not an available adapter!"));
        }
        else
        {
            //no suitable D3D12 adapter - so remove the GFXFlag for D3D12
            
            //FLAGS::GFX_Flags t_flag = App::GFXflags & (~FLAGS::D3D12);
            //changeconst(&const_cast<WUIF::FLAGS::GFX_Flags&>(App::GFXflags), &t_flag);
        }
    }
    ...
    /*on less than Win 10 if we include feature levels for 12 we will get an E_INVALIDARG return on
    D3D11CreateDevice*/
...
void DXGIResources::CreateSwapChain()
{
    if (dxgiSwapChain1)
    {
        ...
        window.GFX->D3D11->d3d11ImmediateContext->OMSetRenderTargets(_countof(nullViews), nullViews, nullptr);
        window.GFX->D3D11->ReleaseNonStaticResources();
        window.GFX->D3D11->d3d11RenderTargetView.Reset();
        window.GFX->D3D11->d3d11BackBuffer.Reset();
        if (App::GFXflags & WUIF::FLAGS::D2D)
        {
            if (window.GFX->D2D->d2dDeviceContext)
            {
                window.GFX->D2D->d2dDeviceContext->SetTarget(nullptr);
            }
            window.GFX->D2D->d2dRenderTarget.Reset();
        }
        dxgiBackBuffer.Reset();
        window.GFX->D3D11->d3d11ImmediateContext->Flush();
        window.GFX->D3D11->d3d11ImmediateContext->ClearState();
        ...
        if (window.property.allowfsexclusive())
        {
        ...
        HRESULT hr = dxgiSwapChain1->ResizeBuffers(
            ...
            window.property.actualwidth(),  //new width of the back buffer
            window.property.actualheight(),	//new height of the back buffer
    ...
    else
    {
        ...
        dxgiSwapChainDesc1.Width = window.property.actualwidth();
        dxgiSwapChainDesc1.Height = window.property.actualheight();
        ...
        if (dxgiSwapChainDesc1.Flags & DXGI_SWAP_CHAIN_FLAG_ALLOW_TEARING)
        {
            ...
            {
                dxgiSwapChainDesc1.Flags &= ~DXGI_SWAP_CHAIN_FLAG_ALLOW_TEARING;
                DebugPrint(_T("WARNING: Variable refresh rate displays not supported/n"));
            }
        }
        if (window.property.allowfsexclusive())
        {
        ...
        HRESULT hr = dxgiFactory->CreateSwapChainForHwnd(
            window.GFX->D3D11->d3d11Device1.Get(), //D3D11 - this is a pointer to the Direct3D device for the swap chain
                                                //D3D12 - this is a pointer to a direct command queue
            window.hWnd,             //The HWND handle that is associated with the swap chain
    ...
    if ((App::winversion >= OSVersion::WIN10) && (window.enableHDR))
    {
        DetectHDRSupport();
    }
